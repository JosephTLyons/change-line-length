...
function changePreferredLineLength (adjustIndexBy)
{
    ...
    if (! indexIsInBounds (currentIndex, preferredLineLengthValues.length - 1))
        currentIndex = 0;

    let newIndex = currentIndex + adjustIndexBy;

    if (indexIsInBounds (newIndex, preferredLineLengthValues.length - 1))
    {
...
function indexIsInBounds (indexToCheck, lastValidIndex)
{
    if (indexToCheck < 0)
        return false;
